...
class Room(val code: String, private val shoe: Shoe = Shoe()) {
    ...
    /** Bumped inside the lock, read outside it by the sweep — hence @Volatile. */
    @Volatile
    ...
    var emptySince: Long? = nowMillis()
        private set

    val version: Long get() = versionValue
    ...
    private fun startDealerTurn() {
        ...
        holeHidden = false
        deadline = nowMillis() + Pace.DEALER_DRAW_MILLIS
    }
    ...
    private fun settleRound() {
        ...
        holeHidden = false
        deadline = nowMillis() + Pace.PAYOUT_MILLIS
    }
    ...
    private fun armDeadline(millis: Long) {
        deadline = if (seats.count { it.occupied } > 1) nowMillis() + millis else null
    }
    ...
    private fun touchEmpty() {
        emptySince = if (isEmpty) (emptySince ?: nowMillis()) else null
    }
    ...
    private fun render(playerId: String?): TableState {
        ...
        return TableState(
            ...
            deadlineIn = deadline?.let { (it - nowMillis()).coerceAtLeast(0) },
            dealer = DealerView(
                cards = visibleDealer,
