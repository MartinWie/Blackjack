 ...
 */
fun tablePage(code: String, local: Boolean = false): String = page(
    pageTitle = if (local) "Blackjack · solo" else "Blackjack · $code",
    scripts = buildList {
        // The engine first: `table.js` reaches for it as soon as it runs.
        if (local) add("/static/vendor/engine.js")
        add("/static/bank.js")
        add("/static/table.js")
    },
) {
    div(classes = "relative mx-auto flex h-[100dvh] w-full max-w-lg flex-col overflow-hidden") {
        ...
        // `local` is the whole difference between the two transports: one talks to
        // the server, the other to a Room compiled into this page.
        attributes["data-mode"] = if (local) "local" else "online"
        ...
        header(classes = "flex items-center justify-between px-4 pt-[max(0.75rem,env(safe-area-inset-top))] pb-2") {
            ...
            button(classes = "rounded-pill bg-black/30 px-4 py-2 text-center") {
                id = "share"
                span(classes = "block text-[10px] uppercase tracking-widest text-chalk/50") {
                    +(if (local) "Playing" else "Table")
                }
                span(classes = "block text-lg font-bold tracking-[0.3em] text-gold") {
                    id = "code"
