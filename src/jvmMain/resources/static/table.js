/**
 * The table. Something else owns the game; this file owns the felt.
 *
 * That something is a `Room` — on the server for a shared table, compiled into this
 * page for a solo one (see `SoloTable.kt`). Both answer the same state, so everything
 * below the transport is one code path.
 *
 * Rendering is keyed — by card key and hand id — so a re-render leaves untouched
 * cards alone and only the genuinely new ones get animated in. That is the entire
 * trick behind the deal animation.
 */
(function () {
    ...
    const local = root.dataset.mode === 'local';
    ...
    // The table this page is playing at. Local tables are a Room in the browser and
    // never touch the network; shared ones are a Room on the server.
    const engine = local
        ? new BJEngine.de.mw.blackjack.SoloTable(window.Bank.playerName())
        : null;

    /** Same call in both worlds: returns the new state, or null after showing why not. */
    async function post(path, body) {
        const answer = local ? await localCall(path, body) : await serverCall(path, body);
        if (!answer.ok) {
            showError(answer.body.error);
            return null;
        }
        return answer.body;
    }
    ...
    async function serverCall(path, body) {
        try {
            ...
            return {ok: false, body: {error: 'no connection to the table'}};
        }
    }
    ...
    async function localCall(path, body) {
        switch (path) {
            case '/join':
                return JSON.parse(await engine.join());
            case '/bet':
                return JSON.parse(await engine.bet(body.amount));
            case '/bet/clear':
                return JSON.parse(await engine.clearBet());
            case '/deal':
                return JSON.parse(await engine.deal());
            case '/action':
                return JSON.parse(await engine.act(body.action));
            default:
                return {ok: false, body: {error: 'unknown action'}};
        }
    }
    ...
    window.addEventListener('pagehide', (event) => {
        if (event.persisted || local) return;
    ...
    /**
     * A local table has no stream to subscribe to: it is ticked here, the way the
     * server's sweep ticks a shared one, and re-rendered when its version moves.
     */
    function runLocally() {
        let lastVersion = -1;
        setInterval(async () => {
            const version = await engine.tick(Date.now());
            if (version === lastVersion) return;
            lastVersion = version;
            const answer = JSON.parse(await engine.state());
            render(answer.body);
        }, 250);
    }
    ...
    if (local) {
        // The Room is new, so whatever the last session had staked no longer exists —
        // hand it back rather than quietly keeping it.
        window.Bank.resetTable(code);
        post('/join').then(render);
        runLocally();
    } else {
