...
(function () {
    ...
    const Bank = {
        ...
        /**
         * Starts a table's ledger over, refunding anything it had staked. A local
         * table is a fresh Room on every load, so a stake from the last visit is
         * money paid for a round that no longer exists.
         */
        resetTable(code) {
            const key = this.ledgerKey(code);
            const ledger = read(key, null);
            if (ledger && ledger.charged > 0) this.cash = this.cash + ledger.charged;
            try {
